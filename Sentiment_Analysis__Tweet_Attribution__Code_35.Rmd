...
# Let's now compute the number of URL instances. 
...
# First, let's load a list of stopwords from the package stopwords.
...
# We'll tackle that later on, in  acompletely different way. 
...
# Keeping tweets_without_urls_and_apostrophes for next code chunk.
...
tab <- tweets_without_urls_and_apostrophes[index, ] %>%
  ...
  `colnames<-`(c("Identifier",
                 "Android Tweet with Remaining ' Marks",
...
tab <- tweets_with_enclosed_punctuation %>%
  ...
  `colnames<-`(c("Device", 
                 ...
                 "Number of Tweets Regularized for Sample Size"))
...
index1 <- str_detect(tweets_without_urls$text, 
  "(\\s\\.(?!\\.)[\\S])|((?!\\.)[\\S]\\.\\s)|(\\s\\.\\s)|((?!\\.)[\\s\\S]\\.$)")
index1 <- which(index1 == T)

index2 <- str_detect(tweets_without_urls$text, 
  "(?!\\.)[\\S]\\.(?!\\.)[\\S]")
index2 <- which(index2 == T)
int <- intersect(index1, index2)

tab <- tweets_without_urls$text[int]
...
df <- tweets_without_urls %>%
  select(text) %>%
  mutate(text = str_replace_all(text, "\\.{2,}", "")) %>%
  mutate(total = str_count(text,  "\\."),
         non = str_count(text, "(\\s\\.)|(\\S\\.)|(\\.$)"),
         enc = str_count(text, "\\S\\.\\S")) %>%
  mutate(dif = total - non - enc) %>%
  select(total, non, enc, dif) %>%
  mutate(num = 1:nrow(tweets_without_urls)) %>%
  select(num, everything())

index <- which(df$dif > 0 | df$dif < 0)
index
...
str_extract_all(tweets_without_urls$text[129],
   ".\\..|\\s\\..|.\\.\\s|((?!\\.)[\\s\\S]\\.$)")             
...
special_sequences <- c("\\\ n", "&amp", "cc:", 
                       ...
                       "Single Dots", 
                       "Single Dots not Enclosed", 
                       "Single Dots Enclosed",
                       "U.S.", "U.S.A.",
                       "L.A., N.Y., S.C., N.H., O.K.",
                       "lyin'", "Lyin'", "Ya'", "ya'", "havn't")
...
tab <- tweets_without_urls %>%
  ...
  `colnames<-`(c("Device",
                 ...
                 "Number of Short Forms Regularized for Sample Size"))
