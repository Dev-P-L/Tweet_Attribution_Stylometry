...
urls_count <- str_count(train_tweets$text, urls_as_argument)
...
mentions_as_argument <- paste(mentions, "|", sep = "", collapse = "")

mentions_as_argument <- str_replace(mentions_as_argument, "\\|$", "")
...
hashtags_as_argument <- paste(hashtags, "|", sep = "", collapse = "")

hashtags_as_argument <- str_replace(hashtags_as_argument, "\\|$", "")

# Tweets without URLS and mentions

text <- str_replace_all(tweets_without_urls$text, 
                        mentions_as_argument,
                        "MENTIONPLACEHOLDER")
...
Let's have a first look at Android tweets, as a first step towards exploratory data analysis. 
...
```{r Frequency of single punctuation marks}
...
# Are added as well two special sequences: numbers followed 
...
There are no percentage marks because combinations of numbers directly followed with the percentage mark have been discarded; they will be treated separately, among special sequences. The same holds, *mutatis mutandis*, for the & mark, which appears only once in the table above because the special sequence *&amp;* has also been discarded and will also be tackled among special sequences. 
...
output <- 
  data.frame(matrix(length(punctuation_marks) * 4, 
                    nrow = length(punctuation_marks), 
                    ncol = 4) * 1) %>%
  `colnames<-`(c("Double Punctuation Mark", 
                 "Number of Instances from Android",
                 "Number of Instances from iPhone",
                 "Number from iPhone Regularized for Sample Size"))
...
output <- 
  data.frame(matrix(length(punctuation_marks) * 4, 
                    nrow = length(punctuation_marks), 
                    ncol = 4) * 1) %>%
  `colnames<-`(c("Triple Punctuation Mark", 
                 "Number of Instances from Android",
                 "Number of Instances from iPhone",
                 "Number from iPhone Regularized for Sample Size"))
...
output <- 
  data.frame(matrix(length(punctuation_marks) * 4, 
                    nrow = length(punctuation_marks), 
                    ncol = 4) * 1) %>%
  `colnames<-`(c("Quadruple Punctuation Mark", 
                 "Number of Instances from Android",
                 "Number of Instances from iPhone",
                 "Number from iPhone Regularized for Sample Size"))
...
# with specific numbers of enclosed punctuation sequences.
...
# Let's move from tidy format to wide format 
# in order to have both devices as column heads.

tab <- 
  spread(tab, device, n) %>%
  `colnames<-`(c("Number of Enclosed Punctuation Sequences per Tweet", 
                 "Number of Android Tweets",
                 "Number of iPhone Tweets"))
...
```
These are data about enclosed punctuation sequences, e.g. in *S.H./N.C./N.Y.* we have five enclosed punctuation sequences: .    ./     .    ./    . 
...
tab <- tweets_without_urls %>%
  ...
  `colnames<-`(c("Device",
                 "Tweet",
                 "Number of Enclosed Punctuation Sequences"))
...
Enclosed punctuation sequences are often about numbers with thousand or decimal separators, about time and date. These will be dealt with under special sequences later on. But, let's first focus on some single enclosed punctuation marks.  
...
list_short_forms_as_argument <- 
  paste(list_short_forms, "|", sep = "", collapse = "")

list_short_forms_as_argument <- 
  str_replace(list_short_forms_as_argument, "\\|$", "")
...
tab <- tweets_without_urls %>%
  select(text, device) %>%
  mutate(text = str_to_lower(text, locale = "en")) %>%
  mutate(n = str_count(text, list_short_forms_as_argument)) %>%
  group_by(device) %>%
  summarise(n = sum(n)) %>%
...
# We already have a list of short forms, whihc has been kept.
...
list_negational_short_forms_as_argument <- 
  paste(list_negational_short_forms, "|", sep = "", collapse = "")

list_negational_short_forms_as_argument <- 
  str_replace(list_negational_short_forms_as_argument, "\\|$", "")
...
list_affirmative_short_forms_as_argument <- 
  paste(list_affirmative_short_forms, "|", sep = "", collapse = "")

list_affirmative_short_forms_as_argument <- 
  str_replace(list_affirmative_short_forms_as_argument, "\\|$", "")
...
v_a <- train_tweets %>%
  select(device, text) %>%
  filter(device == "Android") %>%
  mutate(urls = str_extract_all(text, urls_as_argument)) %>%
  select(urls)
...
# Counting number of tweets with 1 or more URLs
...
# Counting number of tweets with just 1 URL
...
# Counting number of iPhone tweets with just 2 URLs
...
# Counting number of tweets with more than 2 URLs
...
# Assembling the 5 sub-tables into a global one.
...
# Table: number of mentions per device
...
# Counts number of tweets with 1 or more mentions.
...
# Counts number of tweets with just 1 mention.
...
# Counts number of iPhone tweets with just 2 mentions.
...
# Counts number of tweets with strictly more than 2 mentions.
...
# Assembles the 5 sub-tables into a global one.
...
The picture is completely different from the URL picture. The Android device predominates but imbalance is limited. Nevertheless, we'll keep the number of mentions per tweet as a candidate predictor. 
...
# Table: number of hashtags per device
...
# Counts number of tweets with 1 or more hashtags.
...
# Counts number of tweets with just 1 hashtag.
...
# Counts number of iPhone tweets with just 2 hashtags.
...
# Counts number of tweets with more than 2 hashtags
