...
train_tweets <- train_tweets %>%
  ...
  mutate(decimal_hour = hour + (minute / 60)) %>%
...
graph <- df %>%
  ...
  theme(plot.title = element_text(hjust = 0.5, vjust = 3, 
        ...
        legend.text = element_text(size = 16, face = "bold",
                                   color = white),
        legend.background = element_rect(fill = sky_blue),
        ...
        # Formats axis ticks.
        axis.ticks.x = element_line(size = 8, color = yellow),
        ...
        plot.background = element_rect(fill = sky_blue)) +
...
building_graph_series <-                          
  function(d, sub_title) {                          
    ...
    # In order to automatically customize the y-axis, 
    # here is a correspondence table between on the one hand 
    # the maximum number of tweets for one  hour during a day and 
    # on the other hand the top grid and the step between grids.
    # We have previously checked that the maximum number of tweets
    # for one hour is 8.
...
for (i in 1:length(hour_slots_duration)) {
  ...
  # Computes line 3 as a percentage of 
