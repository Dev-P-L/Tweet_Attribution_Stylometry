...
graph <- train_tweets %>%
  ...
  theme(plot.title = element_text(hjust = 0.5, vjust = 3, 
        ...
        panel.grid.major.y = element_line(colour = sky_blue,
                                          size = 2),
        panel.grid.minor.y = element_line(colour = sky_blue,
                                          size = 2),        
...
Clear-cut difference: more than 50 % in am for Android, i.e. more than twice as much than iPhone. Would we have the same picture per hour in the 24-hour clock?
...
graph <- train_tweets %>%
  ...
  theme(plot.title = element_text(hjust = 0.5, vjust = 3, 
        ...
        panel.grid.major.y = element_line(colour = yellow,
                                          size = 2),
        panel.grid.minor.y = element_line(colour = yellow,
                                          size = 2),
...
# Scale of x tick marks to be incorporated 
# into ggplot2 graphs

seq_ticks_x <- seq(min(train_tweets$hour), 
                   max(train_tweets$hour), 4)

# Creates function building_graph_series to produce 
# series of daily time schedule graphs. The arguments 
# are d and sub_title. d is the sample of days 
# that graphs have to be produced for. 
# sub_titlte is a set of titles for the graphs 
# to be produced. 
...
building_graph_series <-                          
  function(d, sub_title) {                          
    ...
    for (i in seq_along(d)) {                       
      ...
      graph <- buffer %>%                                                             ggplot(aes(hour, n, color = device)) +
        ...
        scale_x_continuous(breaks = seq_ticks_x, 
                           labels = waiver()) +
        ylim(min(buffer$n) - 1, max(buffer$n) + 1) +
    ...
    # Makes the graph interactive.
    graph <- ggplotly(graph, height = 360)
...
# marrangeGrob(l, nrow = 1, ncol = 2, top = "") 
htmltools::tagList(l)
}
```
...
# Let's select 4 days at random. 

set.seed(1)
sample_r <- sample(days, sample_size, replace = FALSE)

# Let's determine the subtitle of the 4 graphs,
...
Let's get broader insights about daily time schedule, using the whole dataset. We've illustrated variety, let's now illustrate proximity. 
