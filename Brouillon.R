...
# after depriving the tweets expurgated from the compact hashtags.
...
# being removed.
stop <- paste(" ", unique(tidytext::stop_words$word), " ", sep = "")
...
# Removing stopwords AGAIN as a minimum for the genitive cases. 
stop <- paste(" ", unique(tidytext::stop_words$word), " ", sep = "")
seq_stop <- seq_along(stop)
exp <- expanded$text
seq_exp <- seq_along(exp)
...
# Let's start from the version with hashstag expansion, i.e. the 
# data frame *expanded*. "All" we need to do is expanding the mentions. 

temp <- data.frame(mention =
        sort(unique(unlist(str_extract_all(expanded$text, "@\\S+")))))
str(temp)

write.csv(temp, "mentions_expanded.csv")
