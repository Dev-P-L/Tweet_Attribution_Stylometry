...
df <- train_tweets %>%
  mutate(index = seq_along(1:nrow(train_tweets))) %>%
  select(device, id_str, created_at)  
...
for (i in 1:nrow(df_android)) {
  date_ref <- df_android$created_at[i]
  temp <- df_iphone %>%
    mutate(diff = difftime(created_at, date_ref, units = "hours"))
  diff_pos <- which(temp$diff > 0 & temp$diff <= 3, arr.ind = TRUE)
  diff_neg <- which(temp$diff < 0 & temp$diff >= -3, arr.ind = TRUE) 
  index <- append(diff_pos, diff_neg) 
  
  if(length(index) > 0) {
      prox <- append(df_android$id_str[i], df_iphone$id_str[index])
