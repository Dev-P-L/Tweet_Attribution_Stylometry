...
```{r Standardizing and expanding hashtags}
# Step 1 - Extracting hastags 
hashtags <- str_extract_all(train_tweets$text, "#\\S+")

# Step 2 - Creating a list of unique hashtags
# hashtags_crude is a list, whose elements are character vectors #`either empty or containing hashtags.  
# Let's concatenate all hastags into one character vector.

hashtags <- paste(unlist(hashtags), sep = " ")

hashtags[673]
unlist(hashtags[673])
w <- paste(unlist(hashtags[673]), sep = " ")
w <- str_replace_all(w, "#", "")
s <- strsplit(w, "(?<=[a-z])(?=[A-Z])", perl = TRUE)
...
```{r Dealing with mentions}
mentions_crude <- str_extract_all(train_tweets$text, "@\\S+")

mentions <- unique(paste(unlist(mentions_crude), sep = " "))
