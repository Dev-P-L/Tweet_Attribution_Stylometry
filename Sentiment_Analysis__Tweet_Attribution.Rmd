...
hashtags_crude <- str_extract_all(train_tweets$text, "#\\S+")
hashtags_crude[96]
hashtags_crude[2135]

# Step 2 - Creating a list of unique hashtags
# hashtags_crude is a list. Each element contains a character vector, #` which is most of the time null because there is no hashtag. 
# In each tweet with one or more hastags, let's amalgamate all 
# hashtags into one string with empty space characters 
# between the different hashtags.

unique <- unique(paste(unlist(hashtags_crude), sep = " "))
...
ind_mix <-                                 # Hashtags with some 
  grep("[A-Z]", unique, perl=TRUE)         # uppercase letters
ind_lower <-                               # Hashtags without 
  setdiff(seq_along(unique), ind_mix)      # uppercase letters

# Preference to hashtags with some uppercase letters above
...
for (i in seq_along(unique)) {
  for (j in seq_along(ind_mix)) {
    if(unique[i] == ind[j])
      {if(ind_lower[i] == tolower(ind_mix[j])) 
      {pref[i] <- ind_mix[j]}
...
ind_mix <-                                 # Hashtags with some 
  grep("[a-z]", pref, perl=TRUE)           # lowercase letters
ind_upper <-                               # Hashtags with all
  setdiff(seq_along(pref), ind_mix)        # uppercase letters

# Preference to hashtags with some lowercase letters above
# hashtags all in uppercase letters
pref <- ind_upper

for (i in seq_along(ind_upper)) {
  for (j in seq_along(ind_mix)) {
    if(ind_upper[i] == toupper(ind_mix[j])) 
      {pref[i] <- ind_mix[j]}
  }
}
