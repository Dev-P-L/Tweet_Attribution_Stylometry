...
The method will also be very simple. Very often nouns encapsulated in hashtags and mentions are capitalized. Consequently hashtags and mentions will be expanded using uppercase letters: hashtags and mentions will be split between a lowercase letter followed by a uppercase letter. 

A small improvement will be added to the method. Some hastags can appear under different capitalization: once with just lowercase letters, once with some uppercase letters and once with all uppercase letters. The form that seems preferable is the one with some lowercase letters and uppercase letters. Consequently, hastags will first be made uniform, giving priority to versions with lowercase and uppercase letters. 
...
hashtags_expanded <- seq_along(hashtags)        # Storage vector for 
                                                # exapnded hashtags
# loop expanding hashtags tweet by tweet
for (i in seq_along(hashtags)) {
  w <- paste(unlist(hashtags[i]), sep = " ")    # Unlisting
...
hashtags_expanded[673]
length(grep("MAGA", hashtags_expanded))

# Hashtags have been expanded on uppercase letters following lowercase letters. There is no splitting on an uppercase letter following another uppercase letter: e.g. "@MakeAmericaGreatAgain" has become "Make America Great Again". In the case of "MAGA", which is an acronym referring to the same expression, it might make sense to expand "MAGA" into "Make America Great Again", which is symbolically the equivalent but cannot be dealt with in the same way if not expanded. Consequently, let's create a second vector called *hashtags_fully_expanded* with "MAGA" being expanded. 
...
# Adding expanded hashtags and fully expanded hashtags to data frame train_tweets. 

train_tweets <- train_tweets %>%
  mutate(hashtags_expanded = hashtags_expanded,
         hashtags_fully_expanded = hashtags_fully_expanded)
```

<br>

Two new features have been added in the data frame *train_tweets*: 
- expanded hastags  
- and fully expanded hashtags.

From a semantic point of view, the expanded hashtags and the fully expanded hashtags are redundant with respect to the non expanded hashtags. 

But, from a sentiment analysis standpoint, the presence of non expanded hashtags does not impact results since there is no match between hashtags and lexicon words. 

From a token frequency standpoint, the presence of non expanded hashtags can be impactful, just as all tokens originating in tweet texts, be it words, abreviations, link placeholders, non expanded hashtags, expanded hashtags, fully expanded hashtags, bigrams, ... and, last but not least, expanded mentions, which will be created now.  
...
```{r Opting for mention variants with a mix of letters}
mentions_per_tweet <- str_extract_all(train_tweets$text, "@\\S+")

# Let's create a vector with all mention variants.
mentions <- unique(paste(unlist(mentions_per_tweet), sep = " "))
mentions <- mentions[order(tolower(mentions))]
mentions

mentions <- gsub("_", "", mentions)
# Step 3 - First iteration of hashstags_unique standardization
# Hashstags with some uppercase letters will be substituted
# for hashtags all lowercase.

# We want to lowercase the first letter in hashtags 
# because it doesn't help separating words from hashtags
# and we want to focus on the presence of uppercase 
# in subsequent positions 
# (from position 2 to the penultimate position).

surrogate <- unique
substr(surrogate, 2, 2) <- substr(tolower(surrogate), 2, 2)

ind <-                                     # At least some 
  grep("[A-Z]", surrogate, perl=TRUE)      # uppercase letters

ind_mix <-
  grep("[a-z]", surrogate[ind], perl=TRUE) # Mix of upper and lower

ind_upper <- setdiff(ind, ind)             # Only upper

ind_lower <-                               # Only upper 
