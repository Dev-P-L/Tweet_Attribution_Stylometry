...
# Preparing a working data frame with just 2 features: device and day.
...
building_graph_series <-                          # Function name
  function(d, sub_title) {                        # Function arguments
    
    l <- list(1)                                  # List for output
    
    for (i in seq_along(d)) {                     # For loop on days 
                                                  # from vector d
      
      buffer <- tweets_per_day %>%                # Filtering data
        filter(day == d[i])                       # for day i.
      
      seq_ticks_y <-                              # y tick marks
        seq(min(buffer$n), max(buffer$n), 1)
      
      graph <- buffer %>%                         # Graph for day i        
        ...
        theme(plot.title = element_text(hjust = 0.5, vjust = 1, 
              ...
              axis.text.y = element_text(size = 12),
              legend.text = element_text(size = 12)) +
              scale_colour_manual(values = duo_Palette) +
              theme(panel.background = element_rect(fill = sky_blue)) 
  
    l[[i]] <- graph                              # Placing graph output
    }                                            # into list.
...
```{r Graph with day schedule from a random sample of days}

  
# Now, let's determine the size of three other samples based on tweet activity.
sample_size <- 3

# Let's build up a sample based on global tweet activity per day.
sample_g <- tweets_per_day %>%
  arrange(desc(n)) %>%
  head(sample_size) %>%
  arrange(day) 
v <- sample %>% .$day
